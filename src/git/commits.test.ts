import { describe, it, expect } from "vitest";

import { parseLog, US, RS } from "./commits.js";

// Fixtures mimicking `git log --name-status`: each commit starts with RS, the
// header on the first line, one "status\tpath" line per file below it.
const c1 =
  `${RS}hashAAA${US}Ana Lima${US}ana@example.com${US}2026-01-02T10:00:00-03:00${US}fix: correct login\n` +
  `M\tsrc/auth.ts\n` +
  `A\tsrc/auth.test.ts\n`;

const c2 =
  `${RS}hashBBB${US}Bia Souza${US}bia@example.com${US}2025-06-01T09:30:00-03:00${US}feat: support a || b in the parser\n` +
  `M\tsrc/parser.ts\n` +
  `D\tassets/logo.png\n`;

const raw = `${c1}\n${c2}`;

describe("parseLog", () => {
  it("returns one Commit per block (ignores the empty part before the 1st RS)", () => {
    expect(parseLog(raw)).toHaveLength(2);
  });

  it("captures the hash of the first commit", () => {
    expect(parseLog(raw)[0]!.hash).toBe("hashAAA");
  });

  it("does not let junk leak into the second commit's hash", () => {
    expect(parseLog(raw)[1]!.hash).toBe("hashBBB");
  });

  it("captures author and email", () => {
    const c = parseLog(raw)[0]!;
    expect(c.author).toBe("Ana Lima");
    expect(c.email).toBe("ana@example.com");
  });

  it("converts the date into a real Date (not a string)", () => {
    const c = parseLog(raw)[0]!;
    expect(c.date).toBeInstanceOf(Date);
    expect(c.date.toISOString()).toBe("2026-01-02T13:00:00.000Z");
  });

  it("keeps a subject with | and does NOT let file lines leak into it", () => {
    // The trap: splitting the whole block on US glues the subject to
    // "\nM\tsrc/...". The subject must be only the first line.
    expect(parseLog(raw)[1]!.subject).toBe(
      "feat: support a || b in the parser",
    );
  });

  it("captures each commit's paths, whatever the status letter", () => {
    expect(parseLog(raw)[0]!.files).toEqual([
      "src/auth.ts",
      "src/auth.test.ts",
    ]);
    expect(parseLog(raw)[1]!.files).toEqual([
      "src/parser.ts",
      "assets/logo.png",
    ]);
  });

  it("keeps a C-quoted path whole, tab and all", () => {
    // git quotes a path holding a control character, so the raw tab never
    // reaches the parser and the first tab is the status separator.
    const odd = `${RS}h${US}Ana${US}ana@x.com${US}2026-01-02T10:00:00Z${US}fix\nM\t"src/we\\tird.ts"\n`;
    expect(parseLog(odd)[0]!.files).toEqual([`"src/we\\tird.ts"`]);
  });

  it("a merge commit carries no paths, and still parses", () => {
    const merge = `${RS}hM${US}Ana${US}ana@x.com${US}2026-01-02T10:00:00Z${US}Merge pull request #1\n`;
    const commits = parseLog(merge);
    expect(commits).toHaveLength(1);
    expect(commits[0]!.files).toEqual([]);
  });

  it("returns an empty list for empty input", () => {
    expect(parseLog("")).toEqual([]);
  });

  it("skips a block whose header is missing fields (no crash)", () => {
    const good = `${RS}h${US}Ana${US}ana@x.com${US}2026-01-02T10:00:00Z${US}fix\n`;
    const truncated = `${RS}h2${US}Bia`; // only 2 of 5 fields
    expect(parseLog(`${good}\n${truncated}`)).toHaveLength(1);
  });

  it("skips a block with an unparseable date", () => {
    const bad = `${RS}h${US}Ana${US}ana@x.com${US}not-a-date${US}fix\n`;
    expect(parseLog(bad)).toEqual([]);
  });
});
